...
def match(word, cL, node, wordList, root):
	...
	if cL == word.length():
		...
		if node.word():
			...
			if solutions:
				#print("match10")
				#print("HEREHHJEHRKJEHHEREHHJEHRKJEHHEREHHJEHRKJEHHEREHHJEHRKJEH!")
				#print(solutions)
				#print(solution)
				for x in solutions:
					x.append(solution)
		...
		# Next character blank
			...
			for x in range(26):
				...
				# Continue with search
					...
					#print("flie", x)
					#print("cl+1", cL+1)
					#print(nextNode, nextNode._depth)
					#print()
					solutions += match(word, cL+1, nextNode, wordList, root)
			#print("matche19")
			return solutions
...
def testing2():
	...
	root = listToTrie(dictionary)
	word1 = Word(4, 0, 0, 1)
	emptyLifoQueue = LifoQueue()

	print(match(word1, 0, root, emptyLifoQueue, root))
